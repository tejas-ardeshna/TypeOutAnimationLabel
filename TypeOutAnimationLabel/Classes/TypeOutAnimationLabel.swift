...
public class TypeOutAnimationLabel: UILabel {
    ...
    var canBackward: Bool {
        return currentReplacableIndex < replaceableStrings.count - 1
    }
    ...
    private func initReplacableStringAtIndex(index: Int) {
        currentReplacableChars = replaceableStrings[index].string.characters.map {String($0)}
        status = .Forward
    ...
    func canAnimateNext() -> Bool {
        switch status {
        case .Backward:
            if currentReplacableIndex == replaceableStrings.count - 1 {
    ...
    var currentReplacableString: NSAttributedString? {
        ...
        return replaceableStrings[currentReplacableIndex].attributedSubstring(from: NSRange(location: 0, length: index))
    }
    ...
    public func animationWithFixedString(fixedString: NSAttributedString, replaceableStrings: [NSAttributedString], typeSpeed: TimeInterval, delay: TimeInterval, completion: (() -> Void)?) {
